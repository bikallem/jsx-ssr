...
let strExpr = s => [%expr
  [%e Exp.constant([@implicit_arity] Pconst_string(s, None))]
];
...
let mapExpression = (mapper, e) =>
      ...
      List.fold_right(
        (attr, acc) =>
          ...
          | (Labelled(attr), value) =>
            ...
            %expr
            [Attribute.attr([%e key], [%e value]), ...[%e acc]];
          | (Nolabel, _)
  ...
  | {
        ...
        Pexp_apply(
          {
            pexp_desc:
              Pexp_ident({
                txt: [@implicit_arity] Ldot(_, "createElement"),
                _,
              }),
            _,
          } as expr,
    ...
    [@implicit_arity] Pexp_apply(expr, args) |> Exp.mk(~loc=pexp_loc);
